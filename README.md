...
The renderer processes two tracks simultaneously during each overlap, using pitch-preserving WSOLA on the incoming region, a return to native tempo, equal-power volume curves, a bass handoff and audible EQ sweeps: the outgoing low-pass closes from 3.2 kHz to 450 Hz, while the incoming high-pass opens from 1.8 kHz to 40 Hz. The outgoing filter reaches full strength over the first 150 ms to avoid an abrupt splice. These filters shape the handoff; they are not reverb. A detected, beat-supported outro also gets a separate, short, 20%-wet stereo convolution reverb whose tail carries into the next track; the plain comparison receives neither effect. Completed WAVs have a -1 dBFS sample-peak ceiling. Outside the overlap and short reverb tail, playback uses the original decoded samples, subject to one global output gain.
