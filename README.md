...
The renderer processes two tracks simultaneously during each overlap, using pitch-preserving WSOLA on the incoming region, a return to native tempo, equal-power volume curves, complementary filter sweeps and a bass handoff. A detected, beat-supported outro also gets a short, 20%-wet stereo convolution reverb whose tail carries into the next track; the plain comparison receives no added effect. Completed WAVs have a -1 dBFS sample-peak ceiling. Outside the overlap and short reverb tail, playback uses the original decoded samples, subject to one global output gain.
